...
export default function LocationCard({
  ...
  const { handleSubmit: handleSubmitUpdate, setValue: setValueUpdate } =
    formUpdateContext;
  location = location || [];
  const locationKiotDefault = location?.filter((item: any) => {
    if (type == "online") {
      return item.isDefaultOnline;
    }
    return item.isDefaultKiot;
  })[0];
  ...
  useEffect(() => {
    async function fetchProvince() {
      ...
      setSelectedLocationCheckout(locationKiotDefault);
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <div className="w-full">
          <div className="border-[1px] w-full rounded-sm p-6 bg-white shadow-lg ">
            ...
            <div className="flex flex-row mt-6">
              <div className="text-lg font-semibold">
                {selectedLocation?.name || "Hung Nguyen"}
                <span className="ml-2">(+84)</span>
                <span className="ml-2">{selectedLocation?.phoneNumber}</span>
              </div>
              <div className="ml-8">
                {`
        ...
        ${selectedLocation?.provinceName}`}
              ...
              <div
                className="text-blue-500 underline text-sm hover:cursor-grab ml-6"
                onClick={() => {
                  setOpen(true);
                }}
              >
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => {
                ...
                setSelectedLocationCheckout(locationSelected);
